...
const TaskItem: React.FC<TaskItemProps> = ({ task, type }) => {
  ...
  const style = {
    ...
    cursor: isDragging ? "grabbing" : "grab", // Fixed cursor declaration
    touchAction: "none",
  };
  ...
  return (
    <li
      ...
      {...listeners}
      {...attributes}
      className="cursor-pointer sm:cursor-grab h-[50px] sm:min:h-[30px] w-full grid grid-cols-12 divide-x-2 p-1"
    >
      <span
        onClick={taskFinished}
        className="col-span-12 sm:col-span-3 flex items-center"
      >
        {/* task icon */}
        {!task.isDone ? (
          <svg
